...
Phase 10C converts Invoice Items into a compact protected staging register with add/edit panels. It uses existing routes, models and staging gates only. It adds no schema change, `db:push`, destructive reset, production database command, production migration, production write gate, live OAuth exchange, Microsoft redirect, network call to Microsoft, real provider secrets, production auth readiness, matter edit/close/archive/delete, invoice approval, official invoice number assignment, PDF generation, invoice sending, statement sending, LLM call, Lexpro sync, payment gateway, Yoco, Payfast, shop, checkout or membership functionality.

Do not build broader product features yet. Do not build standalone matter CRUD, matter edit/close/archive/delete, public document links/storage, document sharing, invoice workflow, statement workflow, WhatsApp automation, Lexpro import/sync, marketing system, outreach system, production auth, production database provisioning, production file storage, or agent runtime yet.
...
Phase 9G clarifies client-general documents versus matter-specific documents in the staging UI only. It adds no schema change, migration, route behavior change, production write, live auth, public storage, invoice workflow, statement workflow, WhatsApp, Lexpro import/sync, payment gateway, Yoco, Payfast, shop, checkout or membership functionality.
Phase 9H adds staging-only matter draft billing lines, draft invoices and client draft statement lines. It adds no production write, live auth, invoice approval, official invoice numbers, PDF generation, sending, payment gateway, WhatsApp, Lexpro import/sync, Yoco, Payfast, shop, checkout or membership functionality.
Phase 10A adds GhostPractice-inspired admin workflow and visual redesign only. It adds no schema change, migration, `db:push`, production write, live auth, invoice approval, official invoice numbers, PDF generation, sending, payment gateway, WhatsApp, Lexpro import/sync, Yoco, Payfast, shop, checkout or membership functionality.
Phase 10B simplifies normal admin navigation to Files and Invoice Items, keeps client pages focused on overview/general documents/matters/statement, keeps matter pages focused on overview/matter documents/notes/billing/draft invoices, and lets active Invoice Items prefill matter billing fields. It adds no schema change, migration, `db:push`, production write, live auth, invoice approval, official invoice numbers, PDF generation, sending, payment gateway, WhatsApp, Lexpro import/sync, Yoco, Payfast, shop, checkout or membership functionality.
Phase 10C changes `/admin/invoice-items` from large cards to a compact list/register with Add Invoice Item and row Edit actions only. It adds no schema change, migration, `db:push`, production write, live auth, invoice approval, official invoice numbers, PDF generation, sending, payment gateway, WhatsApp, Lexpro import/sync, Yoco, Payfast, shop, checkout or membership functionality.
...
Phase 9B adds Railway-staging-only document uploads and reusable billing template create/edit behind `BURGESS_STAGING_DOCUMENT_UPLOADS_ENABLED=true` and `BURGESS_STAGING_BILLING_ITEMS_ENABLED=true`. It adds no `db:push`, destructive reset, production database command, production migration, production write gate, live Entra auth, document download/public storage, matter CRUD, invoice/statement approval or sending, LLM call, WhatsApp, Lexpro sync, payment gateway, Yoco, Payfast, shop, checkout or membership functionality.
Phase 9C adds Railway-staging-only protected document view/download for uploaded test documents behind `BURGESS_STAGING_DOCUMENT_UPLOADS_ENABLED=true` and the staging admin password session. It adds no schema change, `db:push`, destructive reset, production database command, production migration, production write gate, live Entra auth, public document URL, public storage, document sharing, matter CRUD, invoice/statement approval or sending, LLM call, WhatsApp, Lexpro sync, payment gateway, Yoco, Payfast, shop, checkout or membership functionality.
Phase 9D adds Railway-staging-only matter creation inside saved client files behind `BURGESS_STAGING_MATTER_WRITES_ENABLED=true` and the staging admin password session. It adds no schema change, `db:push`, destructive reset, production database command, production migration, production write gate, live Entra auth, standalone matter creation, matter edit/close/archive/delete, invoice/statement approval or sending, LLM call, WhatsApp, Lexpro sync, payment gateway, Yoco, Payfast, shop, checkout or membership functionality.
...
- Phase 9B document uploads and billing item edits are limited to Railway staging test data only. Use clearly marked test files/templates, keep production writes disabled, and keep invoice/statement approval, sending, LLM, Lexpro and WhatsApp paths blocked.
- Phase 9C document view/download is limited to already-uploaded Railway staging test documents only. Keep document access private, audited and admin-session protected; do not add public links or sharing.
- Phase 9D matter creation is limited to Railway staging test matters inside saved client files only. Use clearly marked test matter data, keep production writes disabled, and keep matter edit/close/archive, invoice/statement approval, sending, LLM, Lexpro and WhatsApp paths blocked.
