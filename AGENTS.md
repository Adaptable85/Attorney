...
Phase 9G clarifies Railway-staging client-general document areas separately from matter-specific document areas only. It may change document labels, help text, category wording and filename guidance. It adds no schema change, route behavior change, `db:push`, destructive reset, production database command, production migration, production write gate, live OAuth exchange, Microsoft redirect, network call to Microsoft, real provider secrets, production auth readiness, matter edit/close/archive/delete, invoice approval, statement sending, LLM call, Lexpro sync, payment gateway, Yoco, Payfast, shop, checkout or membership functionality.
