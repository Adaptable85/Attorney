...
Phase 10B simplifies the protected staging admin UI around Files, matters inside files, and reusable Invoice Items. It uses existing routes, models and staging gates only. It adds no schema change, `db:push`, destructive reset, production database command, production migration, production write gate, live OAuth exchange, Microsoft redirect, network call to Microsoft, real provider secrets, production auth readiness, matter edit/close/archive/delete, invoice approval, official invoice number assignment, PDF generation, invoice sending, statement sending, LLM call, Lexpro sync, payment gateway, Yoco, Payfast, shop, checkout or membership functionality.
