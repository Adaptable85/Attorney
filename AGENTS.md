...
Phase 9D enables Railway-staging-only matter creation inside saved client files. It may create and list staging test matters only when `BURGESS_STAGING_MATTER_WRITES_ENABLED=true`, `DATABASE_URL` is configured for the approved staging database, and the current user is the staging admin password session. It adds no schema change, `db:push`, destructive reset, production database command, production migration, production write gate, live OAuth exchange, Microsoft redirect, network call to Microsoft, real provider secrets, production auth readiness, standalone matter creation, matter edit/close/archive/delete, invoice approval, statement sending, LLM call, Lexpro sync, payment gateway, Yoco, Payfast, shop, checkout or membership functionality.
