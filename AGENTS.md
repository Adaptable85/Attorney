...
Phase 9E enables Railway-staging-only matter document upload/view/download and legal timeline notes for saved staging matters. It may create staging matter documents and timeline notes only when the relevant staging gates are enabled, `DATABASE_URL` is configured for the approved staging database, and the current user is the staging admin password session. It adds no schema change, `db:push`, destructive reset, production database command, production migration, production write gate, live OAuth exchange, Microsoft redirect, network call to Microsoft, real provider secrets, production auth readiness, matter edit/close/archive/delete, invoice approval, statement sending, LLM call, Lexpro sync, payment gateway, Yoco, Payfast, shop, checkout or membership functionality.
