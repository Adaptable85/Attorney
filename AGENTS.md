...
Phase 9C enables protected Railway-staging document view/download for uploaded test documents inside client files. It may read private test document content only when `BURGESS_STAGING_DOCUMENT_UPLOADS_ENABLED=true`, `DATABASE_URL` is configured for the approved staging database, and the current user is the staging admin password session. It adds no `db:push`, destructive reset, production database command, production migration, production write gate, live OAuth exchange, Microsoft redirect, network call to Microsoft, real provider secrets, production auth readiness, public document URL, public storage, document sharing, matter CRUD, invoice approval, statement sending, LLM call, Lexpro sync, payment gateway, Yoco, Payfast, shop, checkout or membership functionality.

Do not build broader product features yet. Do not build matter CRUD, public document links/storage, document sharing, invoice workflow, statement workflow, WhatsApp automation, Lexpro import/sync, marketing system, outreach system, production auth, production database provisioning, production file storage, or agent runtime yet.
