...
- Phase 9A client-file creation is staging-test-only. It may create minimal client/contact/audit/timeline records in Railway staging only; all other writes, uploads, approvals, live auth, production writes and integrations remain blocked.
- Phase 9B document uploads and reusable billing template edits are staging-test-only. They may store private test document content and billing templates in Railway staging only; matter CRUD, invoice/statement approval or sending, LLM calls, live auth, production writes and integrations remain blocked.
- Phase 9C document view/download is staging-test-only. It may open or download already-uploaded private test documents from protected client file routes only; public links, public storage, document sharing, production document storage, matter CRUD, invoice/statement approval or sending, LLM calls, live auth, production writes and integrations remain blocked.
- Phase 9D matter creation is staging-test-only. It may create matters inside saved client files only; standalone matter creation, matter edit/close/archive/delete, invoice/statement approval or sending, LLM calls, live auth, production writes and integrations remain blocked.
