...
- Create forms must remain disabled until a separate UI-write phase is accepted. Phase 9A accepts only staging client-file creation, Phase 9B accepts only staging document upload plus reusable billing template create/edit, and Phase 9C accepts only protected staging document view/download; none authorize production writes or other UI saves.
