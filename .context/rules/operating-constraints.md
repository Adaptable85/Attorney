...
- Phase 9F may improve admin form layout, labels, spacing and scanability only; it must not add new routes, schema changes, gates, write capabilities or integrations.
- Phase 9G may clarify client-general document wording, categories and filename guidance separately from matter-specific document wording only; it must not add schema changes, migrations, route behavior changes, gates, production writes, live auth, public document links or integrations.
- Phase 9H may create Railway-staging draft matter billing lines, draft invoices and client draft statement lines only behind `BURGESS_STAGING_MATTER_INVOICES_ENABLED=true`; it must not approve invoices, assign official invoice numbers, generate PDFs, send statements, enable payments, run migrations, enable production writes, enable live auth or sync integrations.
- Phase 10A may redesign the protected staging admin UI into dense practice-file and matter-centric workflows only; it must not change schema, run migrations, run `db:push`, enable production writes, enable live auth, approve invoices, assign official invoice numbers, generate PDFs, send statements, enable payments, call LLMs, automate WhatsApp or sync Lexpro.
- Phase 10B may simplify the protected staging admin UI to Files and Invoice Items navigation, client-focused file sections, matter-focused work sections and invoice-item dropdown prefill only; it must not change schema, run migrations, run `db:push`, enable production writes, enable live auth, approve invoices, assign official invoice numbers, generate PDFs, send statements, enable payments, call LLMs, automate WhatsApp or sync Lexpro.
- Phase 10C may convert Invoice Items to a compact list/add/edit UI only; it must not change schema, run migrations, run `db:push`, enable production writes, enable live auth, approve invoices, assign official invoice numbers, generate PDFs, send statements, enable payments, call LLMs, automate WhatsApp or sync Lexpro.
...
- Client general documents belong in the client file document area and include ID, proof of address, FICA, company registration, authority / mandate and general correspondence. Matter-specific documents belong inside the relevant matter.
- Matter invoices start from matter billing lines. Client statements collect matter draft invoices as draft-only statement lines until owner/principal approval and sending workflows are explicitly accepted.
