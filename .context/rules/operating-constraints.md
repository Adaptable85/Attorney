...
- Matters Review pages must remain private. Phase 9D may list/search saved staging test matters and open new matters from inside a saved client file only; they must not accept real matter data or expose active edit, close, archive, upload, approval, save or unrelated submit controls.
...
- Create forms must remain disabled until a separate UI-write phase is accepted. Phase 9A accepts only staging client-file creation, Phase 9B accepts only staging document upload plus reusable billing template create/edit, Phase 9C accepts only protected staging document view/download, and Phase 9D accepts only staging matter creation inside saved client files; none authorize production writes or other UI saves.
