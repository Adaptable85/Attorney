...
Phase 8C read-only Clients Review module is implemented with demo-only client records and demo detail previews, without deployment, database commands, live auth, UI saves, production writes or client/matter CRUD.
...
- Protected `/admin/matters` and `/admin/matters/[id]` read-only pages use safe demo repositories.
...
- Phase 8C Clients Review module: private read-only demo UI only; no deploy, migration, `db:push`, real client data entry, live auth, UI save, production write, payment gateway, invoice workflow, WhatsApp, Lexpro import or email sending is exposed.
