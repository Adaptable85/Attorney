...
import { getStagingClientDocumentContent } from "@/server/staging-documents";

function safeHeaderFilename(filename: string): string {
  return filename.replace(/["\r\n]/g, "_");
}

export async function createDocumentResponse(options: {
  clientId: string;
  documentId: string;
  ...
  if (!access.allowed || !access.principal) {
    return new NextResponse("Admin sign-in is required before opening staging documents.", {
      status: 401
    });
  }

  if (!hasDatabaseUrl()) {
    return new NextResponse("DATABASE_URL is required before opening staging documents.", {
      status: 503
    });
  }
